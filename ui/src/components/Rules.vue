            ...
            <div class="col-lg-10">
              ...
              de patrouilles (<span v-for="(c, index) in playerCompanies" :key="c.id">
                <strong :style="{ color: c.color }">{{ c.name }}</strong><span v-if="index < playerCompanies.length - 2">, </span><span v-else-if="index === playerCompanies.length - 2"> en </span>
                        ...
                        <span v-if="hasNpc">
              én tegen
              <span v-for="(npc, index) in npcCompanies" :key="npc.id">
                <strong :style="{ color: npc.color }">{{ npc.name }}</strong><span v-if="index < npcCompanies.length - 2">, </span><span v-else-if="index === npcCompanies.length - 2"> en </span>
              </span>
            </span>
                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <ul>
                            ...
                            <li v-if="hasNpc">
                                <strong>5 aandelen</strong> van elke AI speler (zoals
                                <span v-for="(npc, index) in npcCompanies" :key="'portfolio-'+npc.id">
                  <strong :style="{ color: npc.color }">{{ npc.name }}</strong><span v-if="index < npcCompanies.length - 2">, </span><span v-else-if="index === npcCompanies.length - 2"> en </span>
                </span>).
                            </li>
                        </ul>
                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        <p>Ieder bedrijf mag zelf beslissen welke opdrachten ze doen en wanneer. Maar let op: <strong>Snelheid is cruciaal!</strong> Elke patrouille heeft <strong>slechts 1 poging</strong> per opdracht.</p>
                ...
                <div v-if="hasNpc" class="card shadow-sm mb-4 border-danger">
                    <div class="card-header bg-danger text-white">
                        <h5 class="mb-0"><i class="bi bi-incognito me-2"></i>6. Niet Spelers Bedrijven (NonPlayerCompanies - NPCS)</h5>
                    </div>
                    <div class="card-body">
                        <p>
                            De volgende partijen doen mee als npc's:
                            <span v-for="(npc, index) in npcCompanies" :key="'ai-'+npc.id">
                <strong :style="{ color: npc.color }">{{ npc.name }}</strong><span v-if="index < npcCompanies.length - 2">, </span><span v-else-if="index === npcCompanies.length - 2"> en </span>
              </span>.
                        </p>
                        <ul>
                            <li><strong>De Benchmark (RNG):</strong> NPC's doen geen taken. Elke minuut rollen zij virtuele dobbelstenen voor "Subsidie" of "Materiaalkosten".</li>
                            <li><strong>De Aasgier:</strong> Faalt jouw patrouille een taak? Dan ruiken de NPC bloed. Ze kunnen je automatisch een meedogenloos <strong>Trade Offer</strong> sturen om jouw aandelen op te kopen met 30% korting terwijl je kwetsbaar bent. Accepteer je de slechte deal voor snel geld, of weiger je?</li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="alert alert-success shadow-sm">
                    ...
                    <ol class="mb-0 fs-5">
                        ...
                        <li>
                            <strong>Investeren:</strong> Koop vroeg in het spel aandelen van partijen
                            <span v-if="hasNpc">
                (inclusief
                <span v-for="(npc, index) in npcCompanies" :key="'win-'+npc.id">
                  <strong :style="{ color: npc.color }">{{ npc.name }}</strong><span v-if="index < npcCompanies.length - 2">, </span><span v-else-if="index === npcCompanies.length - 2"> of </span>
                </span>!)
              </span>
                            waarvan je denkt dat ze veel gaan verdienen.
                        </li>
